...
let weather = {
  ...
  displayWeather: function(data) {
    ...
    element("#w_icon").innerHTML = `<img src="http://openweathermap.org/img/wn/${icon}@4x.png" alt="weather-icon">`;
    element("#feels").innerText = feels_like + " °C";
    ...
    element("#min_temp").innerText = temp_min + "°C";
    element("#max_temp").innerText = temp_max + "°C";
  ...
  search: function() {
    this.fetchWeather(element(".srch_bar").value);
  },
  ...
  handleErrors: function(error) {
    ...
    element("#country").innerHTML = `<img class="error_img" src="./img/error.png">`;

    if (!navigator.onLine) {
      showError("you are offline");
    } else if (element(".srch_bar").value == "") {
...
element(".srch button").onclick = () => weather.search();

element(".srch_bar").onkeyup = (event) => {
